...
const RolesTable = ({ data }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const groupsResponse = await axios.get('/roles');
    ...
    const columns = useMemo(() => [
     
        {
            ...
            header: 'Role Title',  // Text to display in the column header.
    ...
    const table = useReactTable({
        ...
        initialState: {
            columnOrder: ['select', 'name', 'member_count',], //customize the initial column order
            columnVisibility: {
                                                ...
                                                <div
                                                    className={
                                                        header.column.getCanSort()
                                                            ? 'cursor-pointer select-none flex items-start justify-start gap-4 p-3 uppercase text-white text-sm'
                                                            : ''
                                                    }
                    ...
                    <tbody className='px-2 bg-gray-700 text-gray-300 overflow-auto'>
                        {
                            table.getRowModel().rows.length ? (
                                table.getRowModel().rows.map(row => (
                                    <tr key={row.id} className='border-b border-gray-500 h-14 p-3'>
