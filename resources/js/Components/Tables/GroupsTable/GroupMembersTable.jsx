...
const GroupMembersTable = ({ membersList }) => {
    ...
    const { membersAdded } = useSelector((store) => store.group.value);

    // const fetchData = async () => {
    //     try {
    //         // const membershipResponse = membersList;
    //         const profilesResponse = await axios.get('/avatars'); // Assuming this endpoint exists

    //         const memberships = membersList;
    ...
    const columns = useMemo(() => [
        ...
        {
            accessorFn: row => `${row.country_phone_code}${row.phone}`, //note: normal non-fuzzy filter column - case sensitive
            id: 'phone',
            cell: info => info.getValue(),
